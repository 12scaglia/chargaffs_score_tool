...
async def parse_fasta_upload(file: UploadFile) -> ParsedFasta:
    """Validate, parse (via Biopython SeqIO) and clean an uploaded FASTA
    file. Only the first record of a (possibly multi-FASTA) file is used.
    """
    ...
    try:
        record = next(SeqIO.parse(text_stream, "fasta"), None)
    finally:
